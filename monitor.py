...
def run():
    ...
    # Guardar alertas pendientes en prices.json para que la web app las muestre
    alert_objects = []
    for route in config.get("routes", []):
        route_id = route["id"]
        if route_id not in prices:
            continue
        p = prices[route_id]
        price_usd = p.get("current_price_usd")
        max_usd   = convert_to_usd(float(route.get("max_price", 9999)), route.get("currency","USD").upper())
        if price_usd is None:
            continue
        hist = p.get("history", [])
        drop = (hist[-2]["price_usd"] - price_usd) if len(hist) >= 2 else 0
        if price_usd <= max_usd or drop >= 20:
            alert_objects.append({
                "id":        route_id,
                "label":     p.get("label",""),
                "origin":    p.get("origin",""),
                "dest":      p.get("best_dest", p.get("scope_value","")),
                "price_usd": price_usd,
                "price_display": p.get("current_price_display",""),
                "best_date": p.get("best_date",""),
                "airline":   p.get("airline",""),
                "source":    p.get("source",""),
                "drop":      round(drop, 2),
                "below_threshold": price_usd <= max_usd,
                "timestamp": now,
            })

    prices["_alerts"] = alert_objects
    prices["_alerts_updated"] = now
